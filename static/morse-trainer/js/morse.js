...
function makeSample(timings, freq) {
	...
	for (var t = 0; t < timings.length; t += 1) {
		...
		for (var i = 0; i < duration; i += 1) {
			next_segment.push(on * Math.sin(i * counterIncrementAmount));
		}

		// Setup Fade
		fade_duration = fadeDuration(freq, SAMPLE_RATE, 4) // 4 periods of sin wave
		// Fade start of segment
		next_segment = fadeSegment(next_segment, fade_duration)
		// Fade end of segment
		next_segment = fadeSegment(next_segment.reverse(), fade_duration).reverse()
		
		// Append audio segment to buf
		buf = buf.concat(next_segment);
		on = 1 - on;
	}
...
function fadeDuration(freq, sampleRate, cycles) {
	return sampleRate * cycles / freq
}

function fadeSegment(segment, duration) {
	return segment.map((value, index, segment) => {
		if (index < duration) {
			return (index / duration) * value
		} else {
			return value
		}
	})
